      ...
      success: function (htl) {
        try {
          ...
          if (html.status != 1) {
            ...
            // If seller just registered, send them to publish page instead of dashboard
            if (window.location.pathname.indexOf("/seller/") !== -1) {
              control.redirect("/publish.html");
            } else {
              control.redirect("../");
            }
          }
        } catch (err) {
          control.popup("Registration failed. Please try again.", W);
          control.html("lgbtn", "Register");
          console.error("Register parse error:", err, htl);
        }
      },
      error: function () {
        control.popup("Unable to reach server. Please try again.", W);
        control.html("lgbtn", "Register");
      },
    });
  }
